...
from point.data.preprocess import load, split
import numpy as np
...
class TestSplit(TestCase):
    ...
    def test_shape(self):
        ...
        train_df, test_df = split(self.df, test_frac=4/11, shuffle=False)

        self.assertEqual(train_df.shape, (7, len(self.feature_values)))
        self.assertEqual(test_df.shape, (4, len(self.feature_values)))
    ...
    def test_values(self):
        ...
        for feature, feature_values in self.feature_values.items():
            ...
            self.assertEqual(list(train_df[feature].values), feature_values[:7])
            self.assertEqual(list(test_df[feature].values), feature_values[7:])
